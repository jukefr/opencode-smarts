...
7. Write failing tests that describe the expected behaviour — run them and confirm they fail
8. Implement the feature following existing patterns exactly — minimum code to make tests pass
9. Run the full test suite — fix every failure
10. Run linter — fix every warning
11. Spawn @reviewer on changed files — fix any Critical findings
...
### Wrap-up
16. Use the `question` tool to ask: "Feature complete on branch `<branch-name>`. Create a pull request?"
    - Options: "Yes, create PR now" / "No, I'll do it later"
17. If yes: run the `/pr` workflow (push + `gh pr create`)
18. Report: branch name, commit hash, files changed, what was built
